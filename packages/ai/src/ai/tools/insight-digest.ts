...
const manageDigestInputSchema = z.object({
	...
	confirmed: z
		.boolean()
		.describe(
			"Required for route, unroute, reschedule, and test. Always call with confirmed=false first to preview the change; only set confirmed=true after the user has explicitly said yes. Ignored for status and preview."
		),
});
