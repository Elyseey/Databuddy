...
export const PerformanceBuilders: Record<string, SimpleQueryConfig> = {
	slow_pages: {
		...
		fields: [
			...
			"AVG(CASE WHEN dom_ready_time > 0 THEN dom_ready_time ELSE NULL END) as avg_load_time",
			"quantileTDigest(0.50)(CASE WHEN dom_ready_time > 0 THEN dom_ready_time ELSE NULL END) as p50_load_time",
		...
		where: ["event_name = 'screen_view'", "path != ''", "dom_ready_time > 0"],
		groupBy: [
	...
	performance_by_country: {
		...
		where: ["event_name = 'screen_view'", "country != ''", "dom_ready_time > 0"],
		groupBy: ["country"],
		orderBy: "p50_load_time DESC",
		limit: 100,
		timeField: "time",
	...
	performance_by_os: {
		...
		where: ["event_name = 'screen_view'", "os_name != ''", "dom_ready_time > 0"],
		groupBy: ["os_name"],
		orderBy: "p50_load_time DESC",
		limit: 100,
		timeField: "time",
	...
	performance_by_region: {
		...
		where: ["event_name = 'screen_view'", "region != ''", "dom_ready_time > 0"],
	...
	load_time_performance: {
		...
		where: ["event_name = 'screen_view'", "dom_ready_time > 0"],
		groupBy: ["toDate(time)"],
		orderBy: "date ASC",
		timeField: "time",
		customizable: true,
